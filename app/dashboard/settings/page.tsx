...
export default function DashboardSettingsPage() {
  ...
  if (!settings) {
    return <div className="flex min-h-[50vh] items-center justify-center"><Loader2 className="animate-spin text-[#5C9EAD]" /></div>;
  }
    ...
    <div className="mx-auto w-full max-w-6xl space-y-5">
      <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-xs text-[#326273]/60">
            Persisted controls applied to payment approvals, treasury allocation, and account security.
          </p>
        </div>
        <button type="button" onClick={() => void save()} disabled={saving} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#073d49] px-5 py-3 text-sm font-black text-white shadow-[0_5px_0_#022b33] transition-transform hover:-translate-y-0.5 disabled:opacity-50">
          {saving ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />}
          Save controls
        </button>
      </header>

      {notice && <div className="rounded-xl border border-[#5C9EAD]/25 bg-[#5C9EAD]/10 px-4 py-3 text-sm font-bold text-[#326273]">{notice}</div>}
      ...
      <section className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="dash-surface p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-black text-[#326273]">Payment controls</h2>
              <p className="text-xs text-[#326273]/55">Limits are enforced as an operating policy and preserved between sessions.</p>
            </div>
          </div>
        ...
        <div className="dash-surface p-6">
          <div className="flex items-center gap-3">
            <LockKeyhole className="text-[#5C9EAD]" />
            <div>
              <h2 className="text-xl font-black text-[#326273]">Security controls</h2>
              <p className="text-xs text-[#326273]/55">Click a control to change the policy, then save.</p>
            </div>
          </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {INFORMATION.map(({ icon: Icon, label, text }) => (
            <div key={label} className="rounded-2xl border border-[#326273]/10 bg-[#F6F0ED] p-5">
              <Icon size={20} className="text-[#5C9EAD]" />
              <div className="mt-3 text-xs font-black uppercase tracking-[0.12em] text-[#326273]/55">{label}</div>
              <p className="mt-2 text-sm font-semibold leading-6 text-[#326273]">{text}</p>
            </div>
          ))}
        </div>
      </section>

      <div className="flex items-center gap-2 text-xs font-semibold text-[#326273]/50">
        <BadgeCheck size={15} className="text-[#5C9EAD]" />
  ...
  return (
    <label className="rounded-2xl border border-[#326273]/10 bg-[#F6F0ED] p-4">
      ...
      <span className="mt-3 flex items-center gap-2">
        <input type="number" min={0} value={value} onChange={(event) => onChange(Number(event.target.value))} className="min-w-0 flex-1 bg-transparent text-2xl font-black text-[#326273] outline-none" />
        <span className="text-xs font-black text-[#5C9EAD]">{suffix}</span>
      </span>
    </label>
  );
}
...
function Toggle({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className="flex w-full items-center justify-between rounded-xl border border-[#326273]/10 bg-[#F6F0ED] p-4 text-left">
      <span className="text-sm font-bold text-[#326273]">{label}</span>
