...
const CONTEXT_RESPONSES: { keywords: string[]; reply: string }[] = [
  ...
  },
  {
    ...
    reply: [
      ...
      '',
      'Batching reduces per-payment spread cost, so a regular run is usually cheaper than ad-hoc transfers. Whether that applies to you depends on your own volume and cadence, which this reply does not read — Transfers shows both.',
      '',
  ...
  },
  {
    ...
    reply: [
      ...
      '',
      'Want me to prepare a move into Smart Treasury?',
    ].join('\n'),
  },
  {
    keywords: ['compliance', 'kyb', 'aml', 'flag', 'limit', 'risk', 'regulatory'],
    reply: [
      '**I will not state your compliance status from memory.**',
      '',
      'Your KYB state, AML flags and remaining daily limit have to be read from',
      'your account. A copilot that answers those from a script can tell you that',
      'you are cleared when nothing has been checked, which is the one mistake',
      'here that is worse than being unhelpful.',
      '',
      'Settings → KYB shows the verified state and who approved it.',
      '',
      'What I can tell you without reading anything, because it is true of the',
      'product rather than of you:',
      '• Money movement unlocks only at KYB state ACTIVE — no other state releases funds.',
      '• Every payout needs a maker and a separate checker; the two cannot be the same person.',
      '• Each settlement is anchored on Sui and recorded to Walrus, so it is auditable after the fact.',
    ].join('\n'),
  },
  {
    ...
    reply: [
      ...
      '• Batch size and cadence are read from your own history on the Batch page, not from here.',
  ...
  },
  {
    ...
    reply: [
      ...
      '• PHP payout path: live-model · seven additional corridors implemented in code',
      '• Recommended live-model path: PHP 56.42',
      '',
